...
def _get_node_text(
        ...
        num_node_elements, node_summary_statistics, node_labels=None
        ):
    return [
        ...
        f"<br>Node labels: {node_labels_rep}"
        for (node_id, pullback_set_label, partial_cluster_label, num_elements,
             node_summary_statistic, node_labels_rep)
        in zip(node_ids, pullback_set_labels, partial_cluster_labels,
               num_node_elements, node_summary_statistics, node_labels)
        ]
...
def _calculate_graph_data(
        ...
        node_color_statistic, n_sig_figs, node_scale, labels=None
        ):
    ...
    if labels is None:
        ...
        for i in range(len(node_ids)):
            nids = graph.vs["node_elements"][i]
            nlabels = labels[nids]
            unique = sorted(list(set(nlabels)))
            node_labels.append(repr(unique))
    plot_options["node_trace"]["hovertext"] = _get_node_text(
        node_ids, pullback_set_ids, partial_cluster_labels,
        num_node_elements, node_colors_round, node_labels
        )
